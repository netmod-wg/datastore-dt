...
The following terms are used in this docuement:

- dynamic configuration: data modeled as being configuration (config
  true in YANG), 
...
subject to validation of semantical constraints.
...
In the <operational-state> datastore, semantical constraints defined
in the data model are not applied.
...
interface is not applied.  Further, the system has received a hostname
