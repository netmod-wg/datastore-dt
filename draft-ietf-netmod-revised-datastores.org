...
The <operational-state> datastore contains all configuration data actually
...
register their names with IANA, along with the RFC number
that defined it, using the registry defined in ^ds-name-reg^.
...
data is put into the control-plane datastore while, in other cases,
an explicit action (e.g., an RPC) may be required to trigger the
application of the control-plane datastore's data.
...
2) define a 'datastore' identity that can be used to identity the
datastore in protocol operations, and 3) define an 'origin' 
identity that can be used for data that originates from the
datastore.

IETF modules MUST be called "ietf-ds-<datastore-name>".

All modules MUST import "ietf-datastores" in order to obtain
...
** The Datastore Name Registry @ds-name-reg@

This document defines a registry for datastore names called
"Network Management Datastore Names".

The registry shall record for each entry:

- the name of the datastore
- a reference to the datastore's documentation (e.g., the RFC number)

Initial assignments for this registry follows:
...
All datastore names in the registry MUST be unique.
...
* Ephemeral Control-Plane Datastore Example @ephemeral-ds-example@
  ...
  datastore using the guidelines provided in ^cp-ds-guidelines^.
...
be owned and standized via the NETCONF working group.
...
review, but the expectation is that this work will be owned and standized via
