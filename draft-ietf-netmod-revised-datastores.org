...
<operational> also contains applied data from dynamic datastores.
...
- dynamic: represents configuration provided by a dynamic datastore.
...
Each datastore represents differing views of these nodes.  The
<running> datastore will hold the configuration provided by the
operator, for example a single BGP peer.  The <intended> datastore
will conceptually hold the data as validated, after the removal of
data not intended for validation and after any local template
mechanisms are performed. The <operational> datastore will show data
...
<candidate>, if the server supports the <candidate> datastore.
Retrieving the peer will return only the user-specified values.
...
have their values.
  ...
  <bgp or:origin="or:intended" xmlns="urn:example:bgp">
    <local-as>64501</local-as>
    <peer-as>64502</peer-as>
    <peer>
      <name>10.1.2.3</name>
...
device.  The <operational> datastore will contain nodes for both
...
datastore will contain the data from <intended>, as well as nodes
