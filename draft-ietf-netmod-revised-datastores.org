...
- running datastore: A configuration datastore holding
  the configuration of the device.  This datastore is commonly
...
- candidate datastore: A configuration datastore that
  ...
  by all protocols or implementations.  This datastore is commonly
  referred to as "<candidate>".
- startup datastore: A configuration datastore holding
  the configuration loaded by the device into the running
  configuration datastore when it boots. A startup datastore may not
  be supported by all protocols or implementations.  This datastore is
  commonly referred to as "<startup>".
- intended configuration data: configuration data that is intended to
...
- intended datastore: A configuration datastore holding
  the complete intended configuration data of the device.  This datastore
  is commonly referred to as "<intended>".
- conventional datastore: One of the following set of configuration
  datastores: <running>, <startup>, <candidate>, and <intended>.
  These datastores share a common schema and NETCONF operations allow
  copying data between these datastores.  The term "conventional" is
  chosen as a generic umbrella term for these datastores.  Other
  conventional datastores may be defined in future documents.
- conventional configuration data: Configuration data that is stored
...
- operational state datastore: A datastore holding the
  complete operational state data of the device.  This datastore
  is commonly referred to as "<operational>".
- dynamic data: Data that is obtained dynamically during the operation
  of a device through interaction with other systems.
...
- applied configuration data: Configuration data that is currently
  used by a device. Applied configuration data consists of conventional
...
** The Startup Datastore (<startup>)

The startup datastore (<startup>) is an optional configuration
...
The candidate datastore (<candidate>) is an optional configuration
...
** The Running Datastore (<running>)

The running configuration (<running>) datastore holds the complete
configuration currently active on the device.  <running> always
exists. It may include inactive configuration or
template-mechanism-oriented configuration that require further
expansion.

** The Intended Datastore (<intended>)

The intended datastore (<intended>) is a read-only configuration
datastore.  It is tightly coupled to <running>.  When data is written
to <running>, the data that is to be validated is also conceptually
...
The conventional datastores are a set of configuration datastores that
share a common schema, allowing data to be copied between them.  The
term is meant as a generic umbrella description of these datastores.
The set of datastores include:
...
** The Operational Datastore (<operational>)

The operational datastore (<operational>) is a read-only datastore that
...
<operational> contains state data and all configuration data actually
used by the system.  This includes all applied data from <intended>,
system-provided configuration, and default values defined by any
supported data models.  In addition, <operational> also contains data
from dynamic datastores.

Changes to configuration data may take time to percolate through to
<operational>.  During this period, <operational>
will return data nodes for both the previous and current
configuration, as closely as possible tracking the current operation
of the device.  Such remnant data from the previous configuration persists
...
As a result of remnant data, the semantic constraints defined in the
data model cannot be relied upon for <operational>, since the system
may have remnant data whose constraints were valid with the previous
configuration and that are not valid with the current configuration.
Since constraints on "config false" nodes may refer to "config true"
nodes, remnant data may force the violation of those constraints.  The
constraints that may not hold include "when", "must", "min-elements",
and "max-elements".  Note that syntactic constraints cannot be
...
Each datastore represents differing views of these data nodes.
<running> will hold the configuration data provided by the user, for
...
device.  <operational> will contain data nodes for both
...
Since the NETCONF <get/> operation returns the contents of <running>
along with the operational state together in one tree, data models
were often forced to branch at the top-level into a config true branch
and a structurally similar config false branch that replicated some of
the config true nodes and added state nodes.  With the datastore
architecture described here, this is not needed anymore since the
different datastores handle the different lifetimes of data objects.
Introducing this architecture makes it possible to write simpler models.
