...
For standards based YANG data modules that have already been
published, or are in active development, that are using split config
and state trees, it is planned that these modules are updated with new
revisions containing the following changes:
...
New standards based YANG data modules should be designed to conform
