...
#+BEGIN_EXAMPLE
  ...
  +----------------+     |        // resources, delays
  | dynamic        |-+   |
  | datastore(s)   | |   |   +-------- learned configuration
...
sources, then the description statement in the YANG moodule should be
...
"learned", even if the learned value is the same as a configured
value.
...
would be reported as "intended" even if a learned value is the same as
