...
- dynamic data: Data that is obtained
  ...
  with other systems and is not persistent.
...
- origin: abstract base identity from which the other identites are
  derived.
...
then the documentation defining the datastore MUST use the
...
identity or one of its derived identites as its base.  This identity
...
be owned and standardized via the NETMOD working group.
...
- notif/rpc/action xpath context to <opertional>
- intended is valid
- no validation in operational
- by default, any rpc/notif/action is validated towards operational,
  but may affect any datastore (eg edit-config).  descriptions can be
...
- A new "supported datastores" capability allows a device to list all
  ...
  they expose, but MUST at least expose the <running>, <intended>, and
  <operational> datastores.  They MAY expose additional datastores,
  such as <candidate> etc.
