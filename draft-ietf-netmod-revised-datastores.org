...
- The same query parameters supported on {+restconf}/data/ are also
 ...
 - 'meta-data' - is a new optional query parameter that filters the
   returned data based on the meta-data annotation.

 - 'with-meta-data' - is a new optional query parameter that
   indicating that the meta-data annotations should be included in the
   reply.
...
meta-data inforation, that is used by this document to annotate data
...
the filtering of nodes returned in a query based on meta-data
