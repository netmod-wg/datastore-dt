...
- learned configuration: Configuration that has been learned via
  control-plane protocol interactions with peer systems.
...
- default configuration: Configuration that is not explicitly provided
  but for which a value is defined in the data model used.
- applied configuration: Configuration that is actively in use by a
  device.  Applies configuration consists of conventional, dynamic,
  learned, system and default configuration.
...
- learned: represents configuration learned from control-plane
  protocol interactions with peer devices.  Examples of control-plane
  protocols that provide learned configuration include DHCP and BGP.
