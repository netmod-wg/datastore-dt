...
report the origin that most accurately reflects the source of the data
that is actively being used by the system.
...
have no configuration or state leaves, only identities and meta-data.
Hence the "YANG module security guidelines" do not apply.
